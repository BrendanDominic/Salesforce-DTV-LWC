...
const GET_DRAFT_PR_QUERY = gql`
    query GetDraftPR($userId: ID) {
        uiapi {
            query {
                ProductRequest(
                    where: { CreatedById: { eq: $userId }, Status: { in: ["Draft", "Submitted", "Received"] } }
...
export default class Dtvscm_productRequest extends LightningElement {
    ...
    wiredDraftPR(value) {
        ...
        if (data) {
            ...
            if (existingPr) {
                const statusValue = existingPr?.Status?.value || 'Draft';
                ...
                if (statusValue === 'Received') {
                    ...
                    this.activePrNumber = '';
                    this.needByDate     = '';
                    console.log('ℹ️ Latest PR is Received — new PR can be created.');
                } else {
